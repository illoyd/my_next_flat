  ...
  class API
    ...
    def check_response_body_errors(response)
      ...
      case response['error_code'].to_i
        ...
        when  7
          suggestions = response['suggestion'] ? "Did you mean #{ response['suggestion'].to_sentence( two_words_connector: ' or ', last_word_connector: ', or ') }?" : nil
          raise UnknownLocationError.new("#{ response['error_string'] } #{ suggestions }".strip, response)
      end
    end
